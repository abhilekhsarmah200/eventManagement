...
adminRouter.post('/adminregister', async (req, res) => {
  const { role, fname, email, password, cpassword } = req.body;

  if (!role || !fname || !email || !password || !cpassword) {
...
// user valid
adminRouter.get('/validadmin', authenticate, async (req, res) => {
  try {
...
adminRouter.get('/adminlogout', authenticate, async (req, res) => {
  try {
    req.rootUser.tokens = req.rootUser.tokens.filter((curelem) => {
      return curelem.token !== req.token;
    });

    res.clearCookie('usercookie', { path: '/admin' });
